...
class image_converter:
    ...
    def callback(self, data):
        ...
        if cols > 60 and rows > 60:
            move, precise = self.stab.calculateCameraDrift(
                cv_image, precision_cut=1)
...
class CameraStabilization():
    ...
    def calculateCameraDrift(self, frame, feature_params=feature_params, show=True, precision_cut=10):
        ...
        if not hasattr(self, 'p0'):  # checks if "p0" is defined
            ...
            if(dist_movement.shape[0] > 1 and dist_movement.shape[1] > 1):
                ...
                dist_movement_accurate, total_std = self.cutOutliers(
                    dist_movement, num_std=1.2)
    ...
    def cutOutliers(self, list, num_std=2):
        ...
        binary_mask_up = list <= list_mean + outlier_cut + 0.1
        # boolean array to exclude random negative movements
        binary_mask_down = list >= list_mean - outlier_cut - 0.1
...
def opt2():
    ...
    while(1):
        ...
        if(img_drone):
            ...
            drone = drone[int(Y_on_screen - np.abs(Y)):,
                          int(X_on_screen - np.abs(X)):]
            ch = cv2.split(drone)  # ch[3] = background mask of the drone
            print(drone.shape)
            print(ch[3].shape)

            roi = img[int(Y_on_screen):int(Y + h),
                      int(X_on_screen):int(X + w), :]
            print(roi.shape)
