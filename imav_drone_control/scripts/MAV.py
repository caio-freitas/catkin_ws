...
class MAV:
    ...
    def chegou(self):
        if (abs(self.goal_pose.pose.position.x - self.drone_pose.pose.position.x) < 0.1) and (abs(self.goal_pose.pose.position.y - self.drone_pose.pose.position.y) < 0.1) and (abs(self.goal_pose.pose.position.z - self.drone_pose.pose.position.z) < 0.10):
            return True
        else:
            return False
    ...
    def RTL(self):
        ...
        while not self.drone_pose.pose.position.z < 0.3:
            rospy.loginfo('Executing State RTL')
