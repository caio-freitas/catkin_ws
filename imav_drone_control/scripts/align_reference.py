...
class adjust_position:
    ...
    pid_x = PID(P=0.001,I=-0.000,D=0.0008)
    pid_y = PID(P=0.001,I=-0.000,D=0.0008)
    pid_z = PID(P=0.001,I=-0.000,D=0.0008)
    pid_x.setPoint(goal_point[0])
    pid_y.setPoint(goal_point[1])
    pid_z.setPoint(goal_point[2])

    def __init__(self):
        #rospy.init_node('align_reference')
	...
	self.return_pub = rospy.Publisher("align_reference/aligned", String, queue_size=1)
