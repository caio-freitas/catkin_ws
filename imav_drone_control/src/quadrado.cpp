/**
 * Quando chega no laço referente ao laço A (ou pode ser um pouco antes), a função Callback para de ser chamada!
	Precisamos entender o porquê de isso acontecer.
	Se dermos o comando
	>> rostopic echo "mavros/local_position/pose" <<
	veremos que o tópico continua enviando mensagens da posição atual do drone!
	Mas não entendi porque a função Callback para de ser chamada.... Enfim, precisamos resolver isso!
*/
...
void set_position(double goal_x, double goal_y, double goal_z){
    ...
    while(position_now[0] != goal_x && position_now[1] != goal_y && position_now[2] != goal_z){
       ...
       if ( (fabs(position_now[0] - goal_x) < ERROR) && fabs(position_now[1] - goal_y) < ERROR && fabs(position_now[2] - goal_z) < ERROR)
           break;
    }    
}
...
void return_to_home(){
    ...
    while(position_now[0] != 0.0 && position_now[1] != 0.0 && position_now[2] != 0.0){
       ...
       if ( (fabs(position_now[0] - 0.0) < ERROR) && fabs(position_now[1] - 0.0) < ERROR && fabs(position_now[2] - 0.0) < ERROR){
...
int main(int argc, char **argv)
{
  ...
  /* Antes de entrarmos no modo offboard, você tem que ter iniciado os setpoints de transmissão. Caso contrário, a escolha do modo de vôo será rejeitada (próx. passo). Aqui, o número 100 foi uma escolha arbitrária */
	...
	if( fabs(position_now[2] - 2.0) < ERROR ){

        printf("\n [ WARN ] Skyrats MAV Takeoff ");

// Posição A = (3, 0, 2);

        set_position(3.0, 0.0, 2.0);

// Posição B = (3, 3, 2)}

        set_position(3.0, 3.0, 2.0);
// Posição C = (0, 3, 2);

        set_position(0.0, 3.0, 2.0);
// Posição D = (0, 0, 2);

        set_position(0.0, 0.0, 2.0);
// Returning to Home
        return_to_home();
